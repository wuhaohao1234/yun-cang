...
class CustomJPasswordField extends StatelessWidget {
  ...
  CustomJPasswordField(this.data, {this.type = 6.0});

  @override
  Widget build(BuildContext context) {
    return CustomPaint(
      painter: MyCustom(data, type: type),
    );
  }
}
...
class MyCustom extends CustomPainter {
  ...
  MyCustom(this.pwdLength, {this.type = 6.0});
...
//   mPwdPaint.setAntiAlias(true);
    ...
    ///  当前有几位密码，画几个实心圆
    for (int i = 0; i < pwdLength.length && i < type; i++) {
      canvas.drawArc(
